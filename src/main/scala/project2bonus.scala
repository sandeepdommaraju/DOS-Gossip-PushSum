...
import PushSum.algo_PS
 ...
 object project2bonus {
     ...
     def main(args: Array[String]) {
          ...
          if (args.length < 4  || !topologies.contains(args(1))  || !algorithms.contains(args(2))) {
          ...
          algorithm match {
              case "gossip"     => algo_G(N, C, R, top, Integer.parseInt(percent))
              case "push-sum"   => algo_PS(N, top, Integer.parseInt(percent))
